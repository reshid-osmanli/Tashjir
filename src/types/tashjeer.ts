...
export type TashjeerLinkKind =
  ...
  | 'SEGMENT_TO_RULE'
  /**
   * إلحاق اختلاف بسطر هدف (FR-ED-06): «العنصر من = طرف RULE بمعرّف اختلاف،
   * إليه = طرف FACE بمفتاح وجه (variantId::alternativeId) داخل السطر الهدف»
   * — مرساة الوجه مستقرة عبر إعادة توليد الأسطر بخلاف معرّف السطر المرئي.
   *
   * الدلالة: بناءة آلية الحافظة (اللصق/النقل الجزئي في سطر)، فيراها المستخدم
   * في لوحة العلاقات ويفككها بحذفها فيعود الاختلاف لسطره الطبيعي. عند العرض
...
export interface TashjeerBranch {
  ...
  side: AnchorSide;
  /**
   * الرتبة الصريحة للاختلاف صاحب الخط (FR-ED-10): تسبق قاعدة المحرك
   * في ترتيب المسارات. غيابها يعني اعتماد الترتيب التلقائي.
   */
  orderRank?: number;
