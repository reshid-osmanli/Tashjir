...
export interface ReadingImam {
  ...
  slug: string;
  /**
   * رقم الترتيب الصريح للظهور (displayOrder) — FR-ED-14، DM-04.
   *
   * رقم واحد صريح لكل إمام بين أقرانه الأئمة (١..ن)، وهو **المصدر الوحيد** الذي
   * يحكم ظهور الإمام في كل الواجهات: بطاقات الرموز بجانب الأسطر، محدد النطاقات،
   * التصفية بقارئ، ترتيب الأمة في التركيب، /quran، /tracking، والتصدير.
   *
   * تغيير الاسم أو الرمز أو إعداد العرض **لا** يغيّره؛ لا يتغير الترتيب إلا
   * بتعديل الرقم نفسه. المعرّف لا يتغير أبدًا بتغير الرقم.
   */
  order: number;
  region?: string;
  /**
...
export interface Narrator {
   ...
   * رقم الترتيب الصريح للظهور (displayOrder) — FR-ED-14، DM-04.
   *
   * رقم واحد صريح لكل راوٍ بين **كل الرواة** (١..ن)، لا داخل إمامه فقط. هو
   * المصدر الوحيد لترتيب الظهور في كل الواجهات وفي التصدير.
   *
   * **ملاحظة إصدار:** قبل إصدار الكتالوج ٢ كان هذا الحقل يعني «ترتيب الراوي داخل
   * إمامه» (١ أو ٢)، وكان ترتيب الظهور الفعلي يؤخذ من `legacyOrderInTayyibah`.
   * البيانات القديمة تُرحَّل تلقائيًا عند القراءة: يُعبَّأ هذا الحقل من ترتيب
   * الطيبة القائم (انظر `normalizeTransmissionCatalog`).
   */
  order: number;
  /** الرمز الذي يُختصر به الراوي في التشجير الكلاسيكي (ب ج د هـ ...) */
  symbol?: string;
  /**
   * ترتيب الراوي في طيبة النشر كما ورد في البذرة (١..٢٠) — مرجع تاريخي.
   *
   * لم يعد يحكم الظهور؛ دوره الآن: (١) الافتراضي الذي يُعبَّأ منه `order` عند
   * ترحيل بيانات قديمة بلا رقم صريح، و(٢) المقابلة العلمية في لوحة التحكم.
   */
...
export interface TransmissionPath {
   ...
   * رقم الترتيب الصريح للظهور (displayOrder) — FR-ED-14، DM-04.
   *
   * أقران الطريق هم طرق راويه، فالرقم ١..ن **داخل الراوي**. موضع الطريق في
   * الترتيب العام مشتق: ترتيب راويه الصريح أولًا ثم رقمه هو بين طرق راويه،
   * فلا يقفز طريق على راوٍ آخر (انظر `pathDisplayKey`).
   */
  order: number;
  depth: number;
  isCanonical: boolean;
