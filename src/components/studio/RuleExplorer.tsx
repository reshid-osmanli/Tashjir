...
/** صف في القائمة الموحّدة: رأس مجموعة أو قاعدة. */
type ExplorerRow =
  | {
      kind: 'group';
      key: string;
  ...
  | { kind: 'rule'; key: string; rule: EngineRule; bucket: ExplorerBucket };
...
const SORT_OPTIONS: Array<{ key: ExplorerSortKey; label: string }> = [
  ...
  { key: 'specificity', label: 'الخصوصية' },
];

const STATUSES: RuleStatus[] = ['DRAFT', 'ACTIVE', 'DISABLED', 'DEPRECATED', 'CONFLICTED', 'EXPERIMENTAL'];
...
export function RuleExplorer({
  ...
  const [query, setQuery] = useState<ExplorerQuery>(EMPTY_EXPLORER_QUERY);
  const [view, setView] = useState<ExplorerView>('buckets');
  const [collapsed, setCollapsed] = useState<Set<ExplorerBucket>>(new Set());
  ...
  // سياق التصفية: التعارضات وعدادات الاستخدام (تُشتق، لا تُخزَّن).
  const context = useMemo(() => {
    if (config) return buildExplorerContext(config);
    return { usage: new Map<string, number>(), conflicts: conflicts ?? new Map<string, string[]>() };
  }, [config, conflicts]);

  const mergedContext = useMemo(
    () => ({ usage: context.usage, conflicts: conflicts && conflicts.size > 0 ? conflicts : context.conflicts }),
    [context, conflicts]
  );

  const visible = useMemo(() => applyExplorerQuery(rules, query, mergedContext), [rules, query, mergedContext]);
  const facets = useMemo(() => explorerFacets(rules, mergedContext), [rules, mergedContext]);
  const groups = useMemo(
    () => (view === 'buckets' ? groupByBucket(rules, query, mergedContext) : []),
    [view, rules, query, mergedContext]
  );

  /** الصفوف المسطّحة (رؤوس مجموعات + قواعد) حتى تعمل نافذة العرض مع التجميع. */
  const rows = useMemo<ExplorerRow[]>(() => {
    if (view === 'flat') {
      return visible.map((rule) => ({ kind: 'rule', key: rule.id, rule, bucket: classifyRule(rule) }));
    }
    const list: ExplorerRow[] = [];
    for (const group of groups) {
      ...
      if (collapsed.has(group.bucket)) continue;
      for (const rule of group.rules) list.push({ kind: 'rule', key: rule.id, rule, bucket: group.bucket });
    }
    return list;
  }, [view, visible, groups, collapsed]);

  const windowed = useWindowedList(listRef, rows.length, { estimateHeight: 74, overscan: 8, threshold: 30 });

  // مؤشرا التمرير (كما في الحزمة ٠٣): يعرف المستخدم أن هناك المزيد.
  useEffect(() => {
  ...
  // الصف المحدد يُرسم دائمًا ويُتمرَّر إليه ولو كان خارج النافذة.
  useEffect(() => {
    selectedRowRef.current?.scrollIntoView({ block: 'nearest' });
  }, [selectedRuleId]);

  const patch = (changes: Partial<ExplorerQuery>) => setQuery((current) => ({ ...current, ...changes }));
  const toggleStatus = (status: RuleStatus) =>
    setQuery((current) => {
      const active = current.statuses === 'ALL' ? [...STATUSES] : [...current.statuses];
      const next = active.includes(status) ? active.filter((item) => item !== status) : [...active, status];
      return { ...current, statuses: next.length === 0 || next.length === STATUSES.length ? 'ALL' : next };
    });
  ...
  return (
    <div className="flex h-full min-h-0 flex-col rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      {/* ==================== القائمة: نافذة عرض + شريط تمرير ظاهر ==================== */}
      <div className="relative min-h-0 flex-1">
        {canScrollUp && (
          <button
            type="button"
            onClick={() => listRef.current?.scrollBy({ top: -320, behavior: 'smooth' })}
            className="absolute start-1/2 top-1 z-20 -translate-x-1/2 rounded-full border border-gray-300 bg-white/95 px-4 py-0.5 text-xs shadow-md hover:bg-gray-50"
            aria-label="الصعود في قائمة القواعد"
          >
            ↑
          </button>
        )}
        {canScrollDown && (
          <button
            type="button"
            onClick={() => listRef.current?.scrollBy({ top: 320, behavior: 'smooth' })}
            className="absolute start-1/2 bottom-1 z-20 -translate-x-1/2 rounded-full border border-gray-300 bg-white/95 px-4 py-0.5 text-xs shadow-md hover:bg-gray-50"
            aria-label="النزول في قائمة القواعد"
          >
            ↓
          </button>
        )}
        <div
          ref={listRef}
          className="h-full min-h-0 overscroll-contain overflow-y-scroll scroll-smooth [scrollbar-gutter:stable] touch-pan-y"
          tabIndex={0}
          aria-label="قائمة القواعد القابلة للتمرير"
        >
          {rows.length === 0 ? (
            <p className="p-6 text-center text-sm text-gray-400">
              {rules.length === 0 ? 'لا قواعد في الملف بعد.' : 'لا قواعد مطابقة للبحث أو التصفية.'}
            </p>
          ) : (
            <ul className="divide-y divide-gray-100">
              {windowed.active && windowed.topPad > 0 && <li aria-hidden style={{ height: windowed.topPad }} />}
              {rows.map((row, index) => {
                const outside = windowed.active && (index < windowed.start || index > windowed.end);
                if (row.kind === 'group') {
                  ...
                  return (
                    <li
                      ...
                      className="sticky top-0 z-[5] bg-gray-50/95 backdrop-blur"
                    >
                      <button
                        type="button"
                        onClick={() =>
                        ...
                        className="flex w-full items-center gap-2 px-4 py-2 text-right"
                        aria-expanded={!collapsed.has(row.bucket)}
                      >
                        <span className="text-xs text-gray-400">{collapsed.has(row.bucket) ? '◂' : '▾'}</span>
                        <span className="text-sm font-bold text-gray-800">{row.label}</span>
                        <span className="rounded bg-white px-1.5 py-0.5 text-xs text-gray-500 ring-1 ring-gray-200">
                          {toArabicDigits(row.count)}
                        </span>
                        {row.conflicted > 0 && (
                          <span className="rounded bg-red-100 px-1.5 py-0.5 text-xs font-medium text-red-700">
                            {toArabicDigits(row.conflicted)} تعارض
                          </span>
                        )}
                        <span className="mr-auto hidden text-xs text-gray-400 md:block" title={BUCKET_HINTS[row.bucket]}>
                          {row.hint}
                        </span>
                      </button>
                    </li>
                ...
                if (outside && row.rule.id !== selectedRuleId) return null;
                return (
                  <RuleRow
                    key={row.key}
                    rowRef={row.rule.id === selectedRuleId ? selectedRowRef : undefined}
                    rule={row.rule}
                    selected={row.rule.id === selectedRuleId}
                    conflictReasons={mergedContext.conflicts.get(row.rule.id)}
                    usage={mergedContext.usage.get(row.rule.id) ?? 0}
                    versionCount={versionCounts?.[row.rule.id] ?? row.rule.version}
                    onMeasure={windowed.active ? (element) => windowed.measure(index, element) : undefined}
                    onSelect={() => onSelect(row.rule.id)}
                    onWhy={onWhy}
