...
describe('ترحيل تصحيحات v8 الموجودة', () => {
  it('يحافظ على ثلاثية التصحيح المسجلة ولا يكررها من لقطة المحرك', () => {
    const doc = baseDocument();
    doc.variants = [{
      id: 'v1', ayahKey: 2004, category: 'MADUD', title: 'النهائي', startPosition: 1, endPosition: 1,
      alternatives: [{ id: 'v1-a', text: 'x', label: 'x', scope: { kind: 'ALL' as const } }],
      status: 'DRAFT', origin: 'ENGINE',
      engineSnapshot: { title: 'اقتراح المحرك', category: 'MADUD', alternatives: [], capturedAt: 't' },
    }];
    doc.corrections = [{
      id: 'corr-1', targetId: 'v1', targetType: 'VARIANT', engineResult: { title: 'A' },
      editorResult: { title: 'B' }, finalResult: { title: 'B' }, at: 't', source: 'editor',
    }];

    const v8 = migrateDocumentToV8(doc);
    expect(v8.corrections).toHaveLength(1);
    expect(v8.corrections[0].id).toBe('corr-1');
    expect(v8.corrections[0].finalResult).toEqual({ title: 'B' });
  });
});

describe('حتمية ترحيل v7', () => {
  it('يعطي نفس معرّفات التصحيح ونطاق العرض عند ترحيل نفس المستند', () => {
    const doc = baseDocument();
    doc.readingWindow = { linkNextAyah: false, focusSegment: { startPosition: 2, endPosition: 4 } };
    doc.variants = [{
      id: 'v-stable', ayahKey: 2004, category: 'MADUD', title: 'مد', startPosition: 2, endPosition: 2,
      alternatives: [], status: 'DRAFT', origin: 'ENGINE',
      engineSnapshot: { title: 'مد', category: 'MADUD', alternatives: [], capturedAt: 't' },
    }];
    const first = migrateDocumentToV8(doc);
    const second = migrateDocumentToV8(doc);
    expect(second.exportedAt).toBe(first.exportedAt);
    expect(second.corrections.map((correction) => correction.id)).toEqual(first.corrections.map((correction) => correction.id));
    expect(second.renderRanges.map((range) => range.id)).toEqual(first.renderRanges.map((range) => range.id));
  });
});
