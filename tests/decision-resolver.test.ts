...
describe('تجاوزات قواعد الدمج المركزية', () => {
  it('قاعدة منع وحيدة تغلب قرار مصفوفة يسمح بالدمج', () => {
    const profile = createDefaultEngineConfig('prevent-only');
    profile.mergeMatrix = [{ a: 'MADD', b: 'TAHQIQ', merge: true, priority: 80, reason: 'مصفوفة تسمح' }];
    profile.rules = [{
      id: 'er-prevent-one', name: 'منع خاص', type: 'MERGE', category: 'MERGE', scope: 'WORD',
      conditions: { all: [
        { field: 'differenceType', op: 'equals', value: 'MADD' },
        { field: 'relatedType', op: 'equals', value: 'TAHQIQ' },
      ] },
      actions: [{ type: 'PREVENT_MERGE' }], priority: 120, groupId: 'merge', specificity: 'WORD',
      hardness: 'HARD', status: 'ACTIVE', version: 1, createdAt: 't', updatedAt: 't',
    }];

    const result = decideMerge('MADD', 'TAHQIQ', profile);
    expect(result.decision.merge).toBe(false);
    expect(result.decision.priority).toBe(120);
    expect(result.trace.some((step) => step.stage === 'RULE_OVERRIDE' && step.status === 'blocked')).toBe(true);
  });

  it('يعامل زوج الدمج بوصفه غير موجّه عند مطابقة قاعدة السياسة', () => {
    const profile = createDefaultEngineConfig('symmetric');
    profile.mergeMatrix = [{ a: 'MADD', b: 'TAHQIQ', merge: true, priority: 80, reason: 'مسموح' }];
    profile.rules = [{
      id: 'er-reverse-prevent', name: 'منع بترتيب عكسي', type: 'MERGE', category: 'MERGE', scope: 'MUSHAF',
      conditions: { all: [
        { field: 'differenceType', op: 'equals', value: 'TAHQIQ' },
        { field: 'relatedType', op: 'equals', value: 'MADD' },
      ] },
      actions: [{ type: 'PREVENT_MERGE' }], priority: 110, groupId: 'merge', specificity: 'MUSHFAF',
      hardness: 'HARD', status: 'ACTIVE', version: 1, createdAt: 't', updatedAt: 't',
    }];

    expect(decideMerge('MADD', 'TAHQIQ', profile).decision.merge).toBe(false);
    expect(decideMerge('TAHQIQ', 'MADD', profile).decision.merge).toBe(false);
  });
});
