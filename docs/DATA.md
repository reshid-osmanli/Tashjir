...
### بيانات Engine Studio المحلية

| المفتاح | المحتوى |
|---|---|
| `tashjeer:engine-studio:v1` | البروفايلات والسياسات والقواعد ومصفوفة الدمج وسجل الاستوديو |

كما يحمل المستند الآن `corrections[]` اختياريا: كل عنصر يحفظ ثلاثية
`Engine=A / Editor=B / Final=B`. تبقى الحقول اختيارية فقط عند قراءة مستند v7؛
أما أي تصحيح جديد لاقتراح محرك فينشئ ثلاثية كاملة قابلة للتتبع والتصدير.
