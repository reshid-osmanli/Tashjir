...
### تعدد الموضع في v8 (DM-09، FR-ED-03 — الحزمة 05)

- **فهرس واحد في المصدرين**: `resolveOccurrenceIndices` في
  `src/lib/tashjeer/multi-difference.ts` هي الدالة نفسها التي يستعملها المحرر
  عند الإنشاء والترحيلُ عند التصدير: المخزّن الصالح يُحتَرم، والمفقود يُشتَق
  بالترتيب ضمن مجموعة (نطاق × موضع مشترك). التجميع باتحاد المواضع (union-find)
  لا بتطابق البداية فقط، فيُعالَج اختلافا الكلمة نفسها بمديين مختلفين تسلسلا
  واحدا.
- **علاقات الموضع**: رابط `DIFFERENCE_TO_DIFFERENCE` يُرحَّل إلى `Relation`
  بطرفين `DIFFERENCE:<المعرف>`، ونوعه من القرار اليدوي: `MUTUALLY_EXCLUSIVE`
  عند «متنافيان» و`RELATED` عند «مرتبطان». العلاقة تُلحَق **بالاختلافين
  معا** في `Difference.relations`، لا بالأول وحده.
- **تصحيحات التنافي**: سطر السجل بإجراء «تصحيح علاقة التنافي» وحقل
  `locusRelation` (قبل = اقتراح المحرك، بعد = قرار المحرر) يُرفَع إلى
  `Correction` بهدفها (معرّف الرابط) وسببها، فتبقى الثلاثية
  Engine/Editor/Final كاملة لكل قرار.
- **الاستدارة**: `buildExportBundle` (مع `v8`) ثم `importDocuments` تحفظ
  المعرفات والفهارس والروابط؛ `migrateVariant` في مخزن المستندات يحفظ الفهرس
  المخزّن الصالح ويُسقِط الفاسد (يُعاد اشتقاقه)، ولا يضيف مفاتيح `undefined`.
